...
# ╔═╡ 0d3c1730-8d21-11ec-31d4-97b061e9995f
...
# ╔═╡ 4910a7d9-6d45-4150-a615-20e2ab231c34
md"# Arena"

# ╔═╡ f82510d3-e630-4a89-880c-4b56143d7bf0
N=5

# ╔═╡ d11226ec-c3a8-4ca4-a643-f5cb43393279
function draw_arena(plt,N)
	offset = 0.5
	#Whole arena
	plot!(plt,[(0+offset,0+offset),(0+offset,N+offset),(N+offset,N+offset),(N+offset,0+offset),(0+offset,0+offset)],label = false,color = :black,lw = 2)
...
begin
	arena = plot(tickfontcolor = :white,minorgrid = false, size = (300,300),xticks = collect(0.5:N+0.5),yticks = collect(0.5:N+0.5))
	draw_arena(arena,N)
	location_mouse = [3,4]
	location_cat = [4,2]
	#Draw arrows
	actions_mouse = [[-1,-1],[-1,0],[-1,1],[0,-1],[0,0],[0,1],[1,-1],[1,0],[1,1]]
	actions_cat = [[0,-1],[0,0],[0,1],[1,-1],[1,0],[1,1]]
	arrow_x_mouse = zeros(length(actions_mouse))
	arrow_y_mouse = zeros(length(actions_mouse))
	arrow_x_cat = zeros(length(actions_cat))
	arrow_y_cat = zeros(length(actions_cat))
	for i in 1:length(actions_mouse)
		mult = 1
		if norm(actions_mouse[i]) > 1
			mult = 1/sqrt(2)
		end
		arrow_x_mouse[i] = actions_mouse[i][1]*mult*1.
		arrow_y_mouse[i] = actions_mouse[i][2]*mult*1.
	end
	for i in 1:length(actions_cat)
		mult = 1
		if norm(actions_cat[i]) > 1
			mult = 1/sqrt(2)
		end
		arrow_x_cat[i] = actions_cat[i][1]*mult*1.
		arrow_y_cat[i] = actions_cat[i][2]*mult*1.
	end
	quiver!(arena,ones(Int64,length(actions_mouse))*location_mouse[1],ones(Int64,length(actions_mouse))*location_mouse[2],quiver = (arrow_x_mouse,arrow_y_mouse),color = "gray",linewidth = 2)
	quiver!(arena,ones(Int64,length(actions_cat))*location_cat[1],ones(Int64,length(actions_cat))*location_cat[2],quiver = (arrow_x_cat,arrow_y_cat),color = "green",linewidth = 2)
	#Draw cat
...
# ╔═╡ c678e977-abf1-4f78-a54c-e0eee0673b81
md"# Occupations"
...
begin
	...
	for i in 1:N, j in 1:N
		if occupations_1[i,j] == 0.0
			occs_1[i,j] = NaN
		else
			occs_1[i,j] = occupations_1[i,j]
		end
		if occupations_0[i,j] == 0.0
			occs_0[i,j] = NaN
		else
			occs_0[i,j] = occupations_0[i,j]
		end
	end
end

# ╔═╡ c0fb9d84-b44f-4db2-9a02-153b26f15f27
sum(occs_1),sum(occs_0)
